...
% Part A: Input data
% ------------------------------------
progVersion  = 'Main_Tension_Stiffening_SingleRun_SimRC_Carol 04/27/2016';   % program version
fname        = 'HRC_model_CS_20.dat';                             % file name to report the output
...
avgST     = 3.8;                                % average matrix strength
totLoad   = 182000;                              % total applied tensile load
...
measL   = 700;                                  % embedment length for measurement in the middle
...
xITF   = [0,  0.02, 0.35,  0.57,  0.87,  1.20,  1.55,  1.97,  2.29,  2.91, 3.65, 4.39, 5.25]*1.2;   % slip
%yITF   = scaling*[0, 10.14, 11.43, 12.29, 12.82, 12.97, 12.44, 11.43, 10.52, 9.42, 8.32, 7.03, 5.84];   % shear stress
%xITF   = [0,  0.35,  0.65,  0.87,  1.20,  1.80,  2.40,  2.77, 3.29, 4.21, 5.65, 7.39, 8.25];   % slip
yITF   = [0, 10.14, 11.43, 12.29, 12.82, 12.97, 12.44, 11.43, 9.52, 8.52, 7.52, 6.52, 5.52]*1.0;   % shear stress
...
Es     = 233000;                            % steel modulus
ns     = 1.05;                                 % number of rebar
ds     = 20;                                % rebar diameter
...
yFIB   = [0, 50, 420,    540,   540];            % stress
maxSfu = 750;                               % maximum fiber stress, if stress>maxStu, terminate program
effEf  = 0.9;                               % Young modulus efficiency factor

% composite and matrix
fc     = 55;                              % compressive strength
Ac     = 150*150;                           % trabritary area of composite per one yarn
rho    = As/Ac;                             % reinforcement ratio Af/Ac
Am     = Ac-As;                            % trabritary area of matrix per one yarn
Em     = 37000; %4735*fc^0.5;                       % young modulus of matrix
...
xSPR   = [0, 0.007, 0.15, 0.40, 1.00];                             % slip, if no spring, set xSPR=[0,1];
...
hasExpr1      = 1;                          % 0=no experimental data, 1=has
...
% N(5,nNode) = kSPR(1);                 % assign nodal springs at spring location 
...
%         end
        ...
        while converge == 0 
            iter = iter + 1;
            s    = Linear_Equation(bc,P(i),h,psi,As,Ef,k,g,nodeSPR);
...
% SP = [strain',P'];
% CS = [ec_cspac(:,1)*1e6, ec_cspac(:,2)];

% If the program terminated before finishing all increments, load array has one
% more element than other responses
...
figure(10)
plot(exprX1*L,exprY1*Ac,'bo'), hold on
plot(D,P,'-k.'), hold on
plot(D,avg_FM,'-g.'), hold on
plot(D,avg_FF,'-r.'),
...
textData    = {'strain';'stress';'CW';'D';'P';'P/Af';'avg_FM';'avg_FF';'avg_SM';'avg_SF';...
               'xITF';'yITF';'SA1';'TA1';'xFIB';'yFIB';'SN1';'SF1';'xSPR';'ySPR';...
